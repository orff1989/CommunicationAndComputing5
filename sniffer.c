...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
    ...
    struct sniff_ethernet *eth;               // The Ethernet header
    struct sniff_ip *ip;                // The IP header
    struct sniff_icmp *icmpp;

  
   
    //Define Ethernet header
    eth = (struct sniff_ethernet *) packet;
     icmpp = (struct sniff_icmp *) header;

    if (ntohs(eth->ether_type) == 0x0800) {
        ip = (struct sniff_ip *)(packet + sizeof(struct sniff_ethernet)+1);
        
        
        printf("%s\n",packet);
        printf("source: %s\n", inet_ntoa(ip->ip_src));
        printf("destination: %s\n", inet_ntoa(ip->ip_dst));
        printf("type: %d\n", ntohs(icmpp->icmp_type));
        printf("code: %d\n", ntohs(icmpp->icmp_code));
    }
